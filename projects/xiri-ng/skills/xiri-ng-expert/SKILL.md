...
**Navigations-Links mit Query-String:** `action: 'link'` sowie alle URL-basierten Navigations-Links (`cardlink.link`, `links`/`list`/`sidenav`/`breadcrumb`-Items, Tabellen-Link-Felder) akzeptieren URLs mit Query-String, z. B. `/Admin/Devices/Table?config=98`. Die interne `xiriUrl`-Pipe trennt den String in Pfad und Query: der Pfad geht an `[routerLink]` (relative Links wie `Test/Query/Page` bleiben relativ zur Route und behalten den Prefix), die Query separat an `[queryParams]` (vorher wurde `?` als literales Segment percent-encodiert → 404; ein an `[routerLink]` gebundener `UrlTree` würde dagegen immer absolut navigieren → Prefix ginge verloren). Kein separates `queryParams`-Feld im JSON nötig — alles steht im URL-String. Gilt **nicht** für `action: 'href'` (externer Link, roher String).
