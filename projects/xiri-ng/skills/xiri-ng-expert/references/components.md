...
Bei `type: 'waiting'` pollt der Dialog eine lang laufende Backend-Aufgabe und öffnet am Ende deren Ergebnis. Der Dialog ist währenddessen nicht schließbar (`disableClose`). Das Poll-Intervall kommt als `checkTime` (ms) aus dem Backend — `dialog.NewDialogWaiting(text, url, header, checkTime, extra, closeText)`; ohne Angabe sind es 2000 ms, und `time` wird als Alt-Feld weiterhin akzeptiert. Gepollt wird per GET auf die mitgegebene URL; sie antwortet mit `NewDialogWaitingNotDone()` (`{done: false}`, weiterpollen), `NewDialogWaitingDone(url, blocked)` (`{done: true, url}`) oder `NewDialogWaitingError(msg)` (`{done: true, error}` — Fehler im Dialog plus Snackbar, Dialog wird wieder schließbar). Schlägt der Poll-Request selbst fehl, wird weitergepollt und das Intervall pro Fehlversuch um 2000 ms bis maximal ~10 s gestreckt.

Bei `done` mit `url` öffnet der Dialog `apiBaseUrl + url` mit `window.open(…, '_blank')` und schließt sich — **jeder Bericht bekommt seinen eigenen Tab**, mehrere Läufe hintereinander überschreiben sich nicht. Ob der Browser den Inhalt inline zeigt oder speichert, entscheidet wie beim Download-Button allein der Content-Type der Antwort. Das `window.open` passiert im Poll-Callback, also **ohne Benutzer-Interaktion** — ein Popup-Blocker greift hier regelmäßig. Dann ersetzt der Dialog seine Buttons durch einen einzelnen „Download"-Button und bleibt offen, der User löst das Öffnen per Klick selbst aus. Der zweite Parameter von `NewDialogWaitingDone` (`blocked`) wird zwar serialisiert, vom Frontend aber nicht ausgewertet.
